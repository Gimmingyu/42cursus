...
/*   Updated: 2022/08/19 15:53:14 by mingkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	monitoring(t_philo *philo)
{
	...
	while (philo->is_finished == FALSE)
	{
		if (philo->condition->must_eat != 0 && \
			philo->total_eat_cnt == philo->condition->must_eat)
		{
...
void	create_thread(t_philo *philo)
{
	...
	while (++idx < philo->condition->nop)
	{
		flag = pthread_create(&philo[idx].thread, NULL, start_routine, philo);
		if (flag)
