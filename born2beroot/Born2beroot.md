...
즉, LVM은 리눅스 안에서 하나의 디스크를 여러 파티션으로 분할해서 파일 시스템을 이용해 특정 디렉터리와 연결시켜 사용한다.
...
이 APT를 준수해 패키지 관리의 목적으로 만들어진 툴이 apt-get, aptitude, apt-cahce 등이다.
